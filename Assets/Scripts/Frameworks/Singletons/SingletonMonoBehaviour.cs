...
namespace Frameworks.Singletons
{
	/// <summary>
	...
	public class SingletonMonoBehaviour<T> : MonoBehaviour where T : MonoBehaviour
	{
		...
		{
			if (!_instance) // 이 클래스의 정적 변수가 비어 있다면, (처음 생성될 때)
			{
			...
			else if (_instance != this) // 정적 변수가 가리키는 객체가 이것이 아니라면, (이미 같은 클래스(컴포넌트)를 가진 게임 오브젝트가 Scene에 있을 때)
			{
